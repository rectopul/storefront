...
/* WEBPACK VAR INJECTION */(function(jQuery) {function changeh6(element) {
    var classes = jQuery(element).attr('class'),
        text = jQuery(element).html();
    jQuery(this).parent().append('<span class="' + classes + '">' + text + '</span>');
    jQuery(this).remove();
}
...
jQuery(document).ready(function ($) {
    ...
    if ($('.nav_logo .nav_logo__menu-wraper > ul > li > ul').length > 0) {
        $('.nav_logo .nav_logo__menu-wraper > ul').each(function (index, element) {
    ...
    $(document).on('click', '.search-form article div.active > ul > li', function () {
        /**
        ...
        if (hasclass && (actClient || datatype || datawhy)) {
            $.post(js_global.xhr_url, data_send, function (data, textStatus, jqXHR) {
                console.log(data);
                if (datawhy) {
                    if (data[0].link) {
                    ...
                    // Two
                    $('.categories__list').html('<li style="order: 0;">Quem você quer revindicar</li>');
                    if (data.length > 0) {
                        for (var t in data) {
                            if (data.hasOwnProperty(t)) {
                                ...
                                $('.categories__list').append('<li data-why="' + slug + '">' + name + '</li>').parent().addClass('active');
                            }
                        }
                    } else {
                        $('.categories__list').html('<li style="order: 0;">Quem você quer revindicar</li>').parent().removeClass('active');
                    }
                } else if (actClient) {
                    $('.list-ptypes').html('<li style="order: 0;">De que tipo</li>');
                    for (var t in data) {
                        if (data.hasOwnProperty(t)) {
                            var _data$t = data[t],
                            ...
                            $('.list-ptypes').append('<li data-type="' + term_id + '">' + name + '</li>').parent().addClass('active');
                        }
                    }
                }
            }, 'json');
        }
        $(this).parent().find('li').css('order', 10);
        $(this).css('order', 0);
    });
});
