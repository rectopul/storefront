...
class ModuleNode extends \MailPoetVendor\Twig\Node\Node
{
    ...
    protected function compileGetParent(\MailPoetVendor\Twig\Compiler $compiler)
    {
        ...
        if ($parent instanceof \MailPoetVendor\Twig\Node\Expression\ConstantExpression) {
            ...
            $compiler->raw('$this->loadTemplate(')->subcompile($parent)->raw(', ')->repr($this->source->getName())->raw(', ')->repr($parent->getTemplateLine())->raw(')');
        }
        $compiler->raw(";\n")->outdent()->write("}\n\n");
    }
    ...
    protected function compileConstructor(\MailPoetVendor\Twig\Compiler $compiler)
    {
        ...
        } elseif (($parent = $this->getNode('parent')) && $parent instanceof \MailPoetVendor\Twig\Node\Expression\ConstantExpression) {
            $compiler->addDebugInfo($parent)->write('$this->parent = $this->loadTemplate(')->subcompile($parent)->raw(', ')->repr($this->source->getName())->raw(', ')->repr($parent->getTemplateLine())->raw(");\n");
        }
