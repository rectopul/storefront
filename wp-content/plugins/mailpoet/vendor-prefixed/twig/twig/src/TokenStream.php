...
class TokenStream
{
    ...
    public function expect($type, $value = null, $message = null)
    {
        ...
        if (!$token->test($type, $value)) {
            $line = $token->getLine();
            throw new \MailPoetVendor\Twig\Error\SyntaxError(\sprintf('%sUnexpected token "%s" of value "%s" ("%s" expected%s).', $message ? $message . '. ' : '', \MailPoetVendor\Twig\Token::typeToEnglish($token->getType()), $token->getValue(), \MailPoetVendor\Twig\Token::typeToEnglish($type), $value ? \sprintf(' with value "%s"', $value) : ''), $line, $this->source);
        }
        $this->next();
        return $token;
    }
